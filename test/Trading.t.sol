...
contract TradingTest is Test {
    ...
    function test_RevertSetFeesIfFeeTooHigh() public {
        vm.startPrank(admin);
            bytes4 selector = bytes4(keccak256("SellFeeTooHigh()"));
            vm.expectRevert(selector);
            trad.setFees(60000, 50000);
        vm.stopPrank();
    }
    ...
    function test_TransferToRouterWithFees() public {
        vm.startPrank(addr1);
            ...
            assertEq(trad.balanceOf(treasuryWallet), balanceTreasuryBeforeTransfer + 5e17);
            assertEq(trad.balanceOf(donationWallet), 5e17);
