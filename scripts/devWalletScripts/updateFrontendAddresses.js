// Script to update the frontend with deployed Don and Donation contract addresses
// This script can be used to update the frontend without deploying new contracts
 ...
 * @param {string} network The network name (e.g., "base-sepolia", "base-mainnet")
 */
async function updateFrontendAddresses(donAddress, donationAddress, network) {
...
// This file is auto-generated by the updateFrontendAddresses script
...
`;
    ...
    const updateComment = `// Updated by updateFrontendAddresses script on ${new Date().toISOString()}`;
    if (!serverConfigContent.includes('// Updated by updateFrontendAddresses')) {
      ...
      serverConfigContent = serverConfigContent.replace(
        /\/\/ Updated by updateFrontendAddresses script on .*/,
        updateComment
      );
    }
...
async function main() {
  try {
    // Try to load addresses from donationAddresses.json
    let donAddress, donationAddress, network;
    
    try {
      const addressesPath = path.join(__dirname, '../donationAddresses.json');
      const addresses = JSON.parse(fs.readFileSync(addressesPath, 'utf8'));
      donAddress = addresses.don;
      donationAddress = addresses.donation;
      network = addresses.network || 'base-sepolia';
      
      console.log(`Loaded addresses from file:`);
      console.log(`DON token: ${donAddress}`);
      console.log(`Donation contract: ${donationAddress}`);
      console.log(`Network: ${network}`);
    } catch (error) {
      console.log('Could not load addresses from file. Please provide them as command line arguments.');
      
      // Check if addresses are provided as command line arguments
      if (process.argv.length >= 4) {
        donAddress = process.argv[2];
        donationAddress = process.argv[3];
        network = process.argv[4] || 'base-sepolia';
        ...
        console.error('Please provide DON and Donation addresses as command line arguments:');
        console.error('node updateFrontendAddresses.js <donAddress> <donationAddress> [network]');
        process.exit(1);
      }
    }
    
    // Update frontend with deployed addresses
    await updateFrontendAddresses(donAddress, donationAddress, network);
  } catch (error) {
